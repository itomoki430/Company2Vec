...
def evaluate_model(model, test_data_x, test_data_y, test_data_industry, test_data_size, mode_classifier = True, max_seq_length = 512, batch_size = 8):
    model = model.eval()
    tr_loss_test = 0
    #score_all = []
    pred_label_test = []
    ...
    for data_index in range(0, len(test_data_x), batch_size):
        ...
        loss = loss_fct(
            #torch.softmax(logits[np.newaxis, :], dim = 1),  
            logits,  
...
def evaluate_model_with_cosine_similarity(model, test_data_x, test_data_y, test_data_industry, test_data_size, 
                                          mode_classifier = True, batch_size = 16, max_seq_length = 512):
    model = model.eval()
    tr_loss_test = 0
    #score_all = []
    pred_label_test = []
